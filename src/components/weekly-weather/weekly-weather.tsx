...
const WeeklyWeather = ({city}: WeeklyWeatherProps) => {
  ...
  const startTime = new Date(date.getFullYear(), date.getMonth(), date.getDate() - 5).toJSON().slice(0,10);

  const fetcher = (url: string) => fetch(url).then((r) => r.json());
  const { data } = useSWR<HistoryResponse>(`/api/history?q=${city}&dt=${startTime}&end_dt=${endTime}`, fetcher);
